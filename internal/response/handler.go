...
func (h *Handler) SyncResponseToSheet(c *echo.Context) error {
	authUserID, ok := shared.GetAuthUserID(c)
	if !ok {
		return shared.RespondError(c, http.StatusUnauthorized, "Unauthorized")
	}

	responseID, err := strconv.ParseInt(c.Param("id"), 10, 32)
	if err != nil {
		return shared.RespondError(c, http.StatusBadRequest, "Invalid response ID")
	}

	response, err := h.service.GetResponseByID(c.Request().Context(), int32(responseID))
	if err != nil {
		return shared.RespondError(c, http.StatusNotFound, "Response not found")
	}
	...
	if err := h.service.SyncResponseManually(c.Request().Context(), int32(responseID)); err != nil {
		if err == ErrSheetsNotConfigured {
			return shared.RespondError(c, http.StatusServiceUnavailable, "Google Sheets integration is not configured")
		}
		if err == ErrNoSheetLinked {
			return shared.RespondError(c, http.StatusBadRequest, "No Google Sheet linked to this form")
		}
		return shared.RespondError(c, http.StatusInternalServerError, "Failed to sync response to sheet")
	}
...
func (h *Handler) SyncAllResponsesToSheet(c *echo.Context) error {
	authUserID, ok := shared.GetAuthUserID(c)
	if !ok {
		return shared.RespondError(c, http.StatusUnauthorized, "Unauthorized")
	}
	...
	syncedCount, err := h.service.SyncAllResponses(c.Request().Context(), int32(formID))
	if err != nil {
		if err == ErrSheetsNotConfigured {
			return shared.RespondError(c, http.StatusServiceUnavailable, "Google Sheets integration is not configured")
		}
		...
		return shared.RespondError(c, http.StatusInternalServerError, "Failed to sync responses to sheet")
	}
